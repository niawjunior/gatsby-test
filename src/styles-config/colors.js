const colors = {
  grey100: '#ECECED',
  grey200: '#CFCFD1',
  grey300: '#B2B3B5',
  grey400: '#78797E',
  grey500: '#3E4047',
  grey600: '#383A40',
  grey700: '#25262B',
  grey800: '#1C1D20',
  grey900: '#131315',
  green: 'rgba(21, 204, 136, 0.95)',
  white: '#ffffff',
  yellow: '#ffdc4e',
  lightYellow: '#f9e892',
  lightBlue: '#697980',
}

export default {
  ...colors,
  textLightest: colors.white,
  textLightestHover: colors.grey200,
  textLight: 'var(--textLight)',
  primary: colors.grey500,
  primaryAlpha: `rgba(32, 35, 42, 0.85)`,
  blueAlpha: `rgba(89, 101, 239, 0.95)`,
  yellowAlpha: `rgba(89, 101, 239, 0.95)`,
  lightGreen: `rgba(5, 227, 104, 0.95)`,
  text: colors.grey500,
  background: '#f4f8fb',
  backgroundArticle: 'var(--bg-post)',
  textTitle: 'var(--textTitle)',
  textMinRead: 'var(--textMinRead)',
  heartFooter: 'red',
  green: colors.green,
  links: colors.yellow,
  backgroundSelection: colors.yellow,
  highlight_code_oneline: '#fff9d9',
  highlight_code_bg: colors.yellow,
  highlight_code_marker: colors.yellow,
  highlight_code_linebg: '#022a4b',
  socialMediaCardFilter: '#437abf',
  postMetadata: 'var(--postMetadata)',
  blockquote: 'var(--blockquote)',
  tagListItemLink: 'var(--tagListItemLink)',
}
